 ...
 * {
 *   ssml: "<speak>…</speak>",
 *   tts: { input:{ssml}, voice:{languageCode:'en-GB',name,ssmlGender:'MALE'}, audioConfig:{audioEncoding:'MP3'} },
 *   "TTS output complete": { …same as tts… }
 * }
 */
router.post('/', express.text({ type: '*/*', limit: '1mb' }), (req, res) => {
  try {
    ...
    return res.json({ ssml, tts: ttsPayload, "TTS output complete": ttsPayload });
  } catch (err) {
